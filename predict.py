...
# Define labels
labels = {
    0: '(', 1: ')', 2: '0', 3: '1', 4: '2', 5: '3', 6: '4', 7: '5', 8: '6', 9: '7',
    10: '8', 11: '9', 12: '=', 13: 'C', 14: '[', 15: ']', 16: '+', 17: 'd', 18: '/',
    19: '∫', 20: '*', 21: '-', 22: 'x', 23: 'y', 24: 'z', 25: '{', 26: '}'
}
...
# NEW: Replace XX pattern with exponentiation
def replace_xx_with_exponent(equation_str):
    result = ''
    i = 0
    while i < len(equation_str):
        if i + 2 < len(equation_str) and equation_str[i + 1] == 'X' and equation_str[i + 2] == 'X':
            base = equation_str[i]
            if i + 3 < len(equation_str) and equation_str[i + 3].isdigit():
                exponent = equation_str[i + 3]
...
def predict(image_path):
    ...
    while i < len(predicted_labels):
        ...
        if (current_label == 15 and i + 1 < len(predicted_labels) and predicted_labels[i + 1] == 15):
            x1, y1, w1, h1 = bounding_boxes[i]
            x2, y2, w2, h2 = bounding_boxes[i + 1]
            if abs(y1 - y2) < 10 and abs(h1 - h2) < 10:
                merged_labels.append(12)  # '='
                skip_next = True
            else:
    ...
    full_equation = replace_xx_with_exponent(lhs_equation)
